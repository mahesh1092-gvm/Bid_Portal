...
import { bidApp } from "./APIs/BidApi.js";
...
app.use("/api/bids", bidApp);
app.use("/api/reviews", reviewApp);
app.use("/api/notifications", notificationApp);

app.get("/api/health", (req, res) => {
  res.status(200).json({ message: "Freelance Bid Portal API is running" });
});
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({ message: "error occurred", error: err.message, stack: err.stack });
});
